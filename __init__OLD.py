...
class VLCWidget(gtk.DrawingArea):
    ...
    def __init__(self, *p):
        ...
        if sys.platform == 'win32':
            ...
            import pyvlc
            import pyvlc.plugins
            vlcpath = os.path.abspath(os.path.dirname(pyvlc.__file__))
            plugin_path= os.path.abspath(os.path.dirname(pyvlc.plugins.__file__))
            print "[00000001] pyvlc vlcpath = "+vlcpath
            print "[00000001] pyvlc plugin_path = "+plugin_path
            #hack to let the media controller know the plugin path AND change temporarily the cwd to the one containing
            # the plugin dir, it seems that it needs it (don't know why")
            cwd = os.getcwd()
            os.chdir(vlcpath)
            self.player=vlc.MediaControl(["--plugin-path=%s"%plugin_path] )
            os.chdir(cwd)
            
        else: self.player=vlc.MediaControl(*p)
        def handle_embed(*p):
            if sys.platform == 'win32':
                xidattr='handle'
            else:
